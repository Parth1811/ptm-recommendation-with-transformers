...
class ModelAutoEncoderTrainer(Trainer):
    ...
    def __init__(self) -> None:
        ...
        dataloader = DataLoader(dataset, batch_size=training_config.batch_size, shuffle=training_config.shuffle)
        optimizer = torch.optim.Adam(model.parameters(), lr=training_config.learning_rate)

        super().__init__(model=model, dataloader=dataloader, optimizer=optimizer, config=training_config)
    ...
    def on_train_begin(self) -> None:
        logger.info("Starting AutoEncoder training for %d epochs on %d samples.", self.config.num_epochs, self.dataset_size)
